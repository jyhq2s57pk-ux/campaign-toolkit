      ...
      <main className="journey-page">
        ...
        <div className="inner-content-wrapper">
          ...
          <div className="journey-content">
            ...
            {/* Filter Tabs — inline under heading */}
            {pages.length > 0 && (
                          ...
                          <div className="content-layout">
                            <div className="image-frame" ref={screenshotRef}>
                              {page.screenshot_url ? (
                                <div className="screenshot-wrapper">
                                  ...
                                  <div className="markers-container">
                                    {pageComponents.map((comp, i) => {
                                      const t = comp.marker_positions?.[0]?.top || ((i * 140) + 58 + 'px');
                                      return (
                                        <div key={comp.id} className={'marker-row ' + (activeComponent?.id === comp.id ? 'active' : '')} style={{ top: t }}>
                                          <MarkerLine />
                                          <div className="number-label"><span className="number-label-text">{comp.marker_number}</span></div>
                                        </div>
                                      );
                                    })}
                                  </div>
                                ...
                                <div className="no-screenshot"><span>No screenshot</span></div>
                              )}
                            </div>
                            ...
                            <div className="descriptions-container">
                              {pageComponents.map((comp) => {
                                ...
                                const hasCopy = comp.copy_ideas?.length > 0;
                                const isCopyExp = expandedCopyIdeas === comp.id;
                                return (
                                  <div
                                    key={comp.id}
                                  ...
                                  >
                                    <div className="feature-content">
                                      <div className="feature-header">
                                        <div className="feature-number"><span className="feature-number-text">{comp.marker_number}</span></div>
                                        <div className="feature-title">{comp.title}</div>
                                      </div>
                                      <div className="feature-description">{comp.description}</div>
                                      <div className="feature-badges">
                                        {badges.map((b, i) => (<Badge key={i} variant={b.variant} size="sm">{b.label}</Badge>))}
                                      </div>
                                      {hasCopy && (
                                        <div className="copy-ideas-section">
                                          <button className={'copy-ideas-toggle ' + (isCopyExp ? 'expanded' : '')} onClick={(e) => toggleCopyIdeas(comp.id, e)}>
                                            <span>Copy Ideas</span>
                                            <span className={'copy-ideas-chevron ' + (isCopyExp ? 'rotated' : '')}><ChevronRight /></span>
                                          </button>
                                          {isCopyExp && (
                                            <div className="copy-ideas-content">
                                              {comp.copy_ideas.map((idea, idx) => (
                                                <div key={idx} className="copy-idea-item"><p>{idea}</p></div>
                                              ))}
                                            </div>
                                          )}
                                        </div>
